# Get recognized journal entries

Retrieves recognized journal entries matching the specified parameters
...
````json
{
  ...
  "paths": {
    "/accounting-api/journal-entries": {
      "get": {
        "operationId": "JournalEntriesController_getRecognized",
        "summary": "Get recognized journal entries",
        "tags": [
          "Accounting (only available for accounting add-on users)"
        ],
        "description": "Retrieves recognized journal entries matching the specified parameters",
        "parameters": [
          {
